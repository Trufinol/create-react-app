...
const pluginClassCreator = ClassConstructor =>
  class PostImportPrefixCleanUp extends ClassConstructor {
    constructor(options) {
      super(options);
    ...
    process(css) {
      ...
      const escapedMark = antDesignTemplateMark.replace(
        /[.*+?^${}()|[\]\\]/g,
        '\\$&'
      );
      ...
      const lines = css.split('\n').map(line => {
        ...
        const replacedLine = `${antDesignTemplateMark} ${line.replace(
          finder,
          ''
        )}`;
...
module.exports = class LessPlugin {
  ...
  install(less, pluginManager) {
    ...
    pluginManager.addPostProcessor(
      new this.PostImportPrefixCleanUp({
        ...this.options,
        less,
      })
    );
